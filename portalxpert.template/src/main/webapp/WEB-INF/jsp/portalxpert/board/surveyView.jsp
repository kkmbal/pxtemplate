				...
				if ($("#surveyExmpl-"+json.surveyNo).parent().attr('id') == 'surveyImg')//이미지
				{
					if(surveyJson.skipPermitYn=='N')//스킵사용 NO
					{
						$("#surveyExmpl-"+json.surveyNo).append(						
							'<div class="imgBox">'
							+'<label class="quest3"><input type="'+strType+'" title="" name="answer-'+json.surveyNo+'" id="exmpl-'+json.exmplNo+'" class="vTop" '+strChecked+'/>'+no+'. </label><span class="sns_img"><a href="#" style="cursor:pointer;" onclick="javascript:fnSurveyImgPop(\''+tempWeb+json.imgName+'\');"><img id="exmplimg-'+json.exmplNo+'" src="'+tempWeb+json.imgName+'" style="width:100%;height:100%;cursor:pointer;" alt="올린이미지"/></a></span>'
							+'</div>'
						);
					}
					else
...
{	
	...
	layerPopupObj.css({'left': left, 'top': top, 'position':'absolute', 'z-index':'10000'});
	layerPopupObj.show();
};
